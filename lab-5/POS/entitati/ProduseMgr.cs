...
namespace entitati
{
    public class ProduseMgr : ProdusAbstractMgr
    {
        ...
        {
                bool exist = produseServicii.Any(item => item.Id == prod.Id && item.Nume == prod.Nume && item.CodIntern == prod.CodIntern && item.Pret == prod.Pret && item.Categorie == prod.Categorie ); 
